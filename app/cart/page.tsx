...
export default function Cart(){
  ...
  return <PageShell><main className="section">
    ...
    {!items.length?<><p className="lead"><LanguageText en="Your bag is empty." ru="Ваша корзина пуста."/></p><Link className="btn" href="/shop"><LanguageText en="CONTINUE SHOPPING" ru="ПРОДОЛЖИТЬ ПОКУПКИ"/></Link></>:<>
      <div className="adminTable" style={{marginTop:30}}>{items.map(item=><div className="adminRow" key={item.id}><div><strong>{language==='ru'?(item.nameRu||item.name):item.name}</strong><small>€{(item.priceCents/100).toFixed(2)} × {item.quantity}</small></div><button className="adminSecondary" type="button" onClick={()=>remove(item.id)}><LanguageText en="REMOVE" ru="УДАЛИТЬ"/></button></div>)}</div>
