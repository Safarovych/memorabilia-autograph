type TranslationResult={nameRu:string;descriptionRu:string};
...
export async function translateProductText(name:string,description:string):Promise<TranslationResult>{
  ...
  if(apiKey){
    ...
    if(response.ok){
      ...
      try{
        ...
        return {nameRu:String(parsed.nameRu||'').trim(),descriptionRu:String(parsed.descriptionRu||'').trim()};
      }catch{}
    }
  }
  const [nameRu,descriptionRu]=await Promise.all([translateText(name),translateText(description)]);
