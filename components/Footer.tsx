...
export default function Footer(){
  ...
  return <footer className="premiumFooter">
    ...
    <div className="premiumFooterNav"><Link href="/">{ru?'Главная':'Home'}</Link><Link href="/shop">{ru?'Магазин':'Shop'}</Link><Link href="/#about">{ru?'О нас':'About'}</Link><Link href="/shop?category=Signed">{ru?'Подлинность':'Authenticity'}</Link><Link href="/#charity">{ru?'Благотворительность':'Charity'}</Link><Link href="/#contact">{ru?'Контакты':'Contact'}</Link></div>
