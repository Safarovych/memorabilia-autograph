...
export default function Site() {
  ...
  return <main className="premiumHome">
    ...
    <section className="categoryStrip">{cats.map((name,i)=>{const ids=['shirts','boots','balls','boxing-gloves','tennis','ufc','formula-1','basketball'];return <Link href={`/shop#${ids[i]}`} key={name} className="categoryPhotoCard"><span className={`categoryPhoto categoryPhoto-${i}`}></span><b>{name}</b></Link>})}</section>
